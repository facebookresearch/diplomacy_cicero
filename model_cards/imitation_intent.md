# imitation_intent model card

## Overview

This model is used to predict the message intent for the next (unobserved) message from power A to power B in a given game situation from power A's point of view. That is, given a game state and dialogue history between A and B, it is trained to predict the annotated intent - a set of orders for power A *and* power B for all phases through the *next* movement phase - for the next message.

The model uses a standard [pre-trained BART model](https://arxiv.org/pdf/1910.13461.pdf). The model was first fine-tuned on the Diplomacy dialogue dataset, and then further fine-tuned on the intent prediction task.
...
```
[input]: S1901M
0 FRANCE -> GERMANY: DMZ BUR?
...
32 GERMANY -> FRANCE: Sounds good.
...
S1901M FRANCE -> GERMANY 5 ANON 5min WTA:
[output]:S1901M
FRANCE: A MAR S A PAR BUR; A PAR BUR; F BRE MAO
GERMANY: A BER KIE; A MUN RUH; F KIE DEN
F1901M
FRANCE: A BUR BEL; A MAR SPA; F MAO POR
GERMANY: A KIE HOL; A RUH BEL; F DEN SWE
```
...
This model only predicts orders for a single power independently (although conditioned on another power's dialogue). The model can only predict the orders past the next movement phase.
